...
- **Últimas verificaciones completas:**
  - Backend: typecheck OK, **158/158 tests** (31 suites).
  - Frontend: build OK, **96/96 tests** (12 archivos de specs).
...
| B3 | `cfde16e` | Gestión de excedentes — redistribución presupuestaria: la propuesta se calcula prorrateando el excedente entre las asignaciones existentes (método del resto mayor, en centavos) y queda limitada a la holgura disponible (total = ingresos netos − asignado). Endpoints `GET /periods/:id/budget/redistribution` (`{ redistribution }`, motivos `SIN_PRESUPUESTO/SIN_EXCEDENTE/SIN_HOLGURA/SIN_ASIGNACIONES`) y `POST /periods/:id/budget/redistribute` (aplica aumentos en transacción, revalidando; 422 `BUDGET_REDISTRIBUTION_NOT_AVAILABLE` si no hay excedente/holgura/asignaciones). UI: panel "Redistribución de excedentes" en la página Presupuesto con tabla de propuesta (actual → propuesto → incremento) y confirmación antes de aplicar. Tests: 10 backend + 5 frontend. |
| B4 | `7477922` | Marcar objetivos como COMPLETADO/CANCELADO: el flujo backend (`POST /objectives/:id/complete|/cancel`) ya existía; este bloque lo consolida con confirmación en la UI (diálogo antes de completar/cancelar, mensaje de éxito/error con feedback del servidor) y cobertura de pruebas: 11 tests backend de transiciones unidireccionales ACTIVE → COMPLETED/CANCELLED (persistencia, doble operación → 422 `GOAL_NOT_ACTIVE`, 403 ajeno, 404 inexistente, aislamiento de aportes/retiros e ingresos vinculados) y 5 tests frontend (confirmación, cancelación, error y etiquetas ACTIVO/COMPLETADO/CANCELADO). Se corrigió además la tabla de endpoints de objetivos en README (PATCH y rutas reales `contributions`/`withdrawals`). Sin retorno a ACTIVO. |
| B5 | *(este commit)* | Finalización de período con sugerencia de nuevo período: el backend (`POST /periods/:id/finalize` ACTIVE→FINISHED + snapshot del informe final en la misma transacción, y `GET /reports/final` con 409 `PERIOD_NOT_FINALIZED`) ya estaba implementado; este bloque mejora la página `/periods/finalize` con confirmación antes de finalizar y, tras el éxito, un panel que sugiere **crear un nuevo período** o **ver el historial** (rutas `/periods/new` y `/periods/history`). Nuevo spec `finalize.spec.ts` (6 tests: carga de ACTIVE con totales, finalizar con confirmación + sugerencias renderizadas, cancelación de confirmación, sin período activo, error del servidor, navegación de los botones). |
...
| B4 — Estados de objetivos | Marcado unidireccional ACTIVE → COMPLETED/CANCELLED: `POST /objectives/:id/complete|/cancel` solo desde `ACTIVE` (422 `GOAL_NOT_ACTIVE` en otro caso); no existe retorno a ACTIVO. Las operaciones de aporte (contributions), retiro (withdrawals) y el vínculo de ingresos (`objetivoId`) se rechazan en objetivos no activos. `PATCH /objectives/:id` no acepta `status` y se mantiene editable (metadatos) tras completar/cancelar, por decisión de alcance. La UI pide confirmación antes de completar/cancelar y muestra el mensaje de resultado/error del servidor; los objetivos no activos ocultan los botones de operación y no aparecen en los selectores de ingreso (solo `ACTIVE`). |
| B5 — Finalización de período | `POST /periods/:id/finalize` es irreversible: ACTIVE → FINISHED con generación del snapshot del informe final en la misma transacción (`snapshots_informes`, índice único por período); el informe final solo se sirve en períodos FINISHED (409 `PERIOD_NOT_FINALIZED`). En la UI de `/periods/finalize` se exige confirmación antes de finalizar y, al completar, se sugiere crear un nuevo período (`/periods/new`) o revisar el historial (`/periods/history`). Fuera de alcance: cancelación vía admin para FINISHED (ya rechazada) y flujo DRAFT/activate (código inerte por decisión M1). |
...
| B4 | completado | Objetivos completados/cancelados con confirmación y cobertura de tests (ver §2 y §3). |
| B5 | completado | Finalización de período con sugerencia de nuevo período (ver §2 y §3). |
